...
template <typename Action_t> static int iterate_over_symbols(const char *filename, Action_t action) {
    ...
    for (; from < fromend; from += size) {
        ...
        if (sym->flags & (BSF_FUNCTION | BSF_GLOBAL)) {
            ...
            action(syminfo.name, elfinfo ? elfinfo->internal_elf_sym.st_size : 0, syminfo.value);
        }
    }
...
NMParser::SymTable NMParser::get_symbols() && {
    SymTable ret;
    iterate_over_symbols(input_file_.c_str(), [&](const char *name, size_t size, size_t addr) {
        Symbol s;
        s.ID = ret.size() + 1;
        s.interal_addr = addr;
        s.name = name;
